...
<template>
  <qr-panel class="field-key-qr-panel" :class="{ legacy: !managed }">
    ...
    <template v-if="fieldKey != null || (username && password)" #body>
      ...
      <p>
        ...
        <sentence-separator/>
        <template v-if="managed">{{ $t('body[1].managed', fieldKey || { displayName: username }) }}</template>
      ...
      <p>
        <span>{{ $t('body[3]', fieldKey || { displayName: username }) }}</span>
        <sentence-separator/>
