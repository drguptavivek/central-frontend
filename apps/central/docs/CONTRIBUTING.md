...
Each component may use one or more of the composables in [`/apps/central/src/composables/`](/apps/central/src/composables/). Most composables will reside in that directory, but if it makes sense to group a composable with other functionality, it may be defined elsewhere. For example, the `useSessions()` composable is defined in [`/apps/central/src/util/session.js`](/apps/central/src/util/session.js).

### Router

We support a number of route meta fields, which we document in [`/apps/central/src/routes.js`](/apps/central/src/routes.js). The router ([`/apps/central/src/router.js`](/apps/central/src/router.js)) contains a fair amount of logic, which is driven largely by the meta fields.
...
The [`usePath()`](/apps/central/src/composables/path.js) composable returns router-related functions. Additionally, some router-related utilities are defined in [`/apps/central/src/util/router.js`](/apps/central/src/util/router.js).

### Response Data

Use the `get` action of the [`request` module](/apps/central/src/store/modules/request.js) of the Vuex store to send a GET request. This will store the response data in the Vuex store, as well as complete common tasks like error handling.

The `get` action may transform the response data, for example, by wrapping the data within a presenter object. These transformations are defined in [`/apps/central/src/store/modules/request/keys.js`](/apps/central/src/store/modules/request/keys.js). In general, `request/keys.js` defines the specific behavior that the `request` module implements for each type of data.

In some cases, different responses may contradict each other, for example, if responses are returned at different times or if there are concurrent users. Some components will use `watchSyncEffect()` to do what they can to reconcile the data. For example, [`ProjectShow`](/apps/central/src/components/project/show.vue) uses `watchSyncEffect()` to reconcile `project` and `forms`.

Some routes require certain conditions to be true about the response data. For example, a route might require the user to be able to perform certain verbs. This is implemented using the [`validateData`](/apps/central/src/routes.js) route meta field.

By default, response data is cleared after the user navigates to a new route. However, exceptions are specified using the [`preserveData`](/apps/central/src/routes.js) route meta field. For example, as the user navigates between project routes, the response data for the project is preserved: a new request for the project is not sent.

### Alerts

We use the [`Alert`](/apps/central/src/components/alert.vue) component to render an alert. Only a single alert is shown at a time &mdash; either at the top of the viewport or, if a modal is shown, at the top of the modal. Either way, the data for the alert is stored in the [`alert` object](/apps/central/src/alert.js) in the container. To display an alert, a component can inject `alert`.

### Internationalization

We use the Vue I18n plugin for internationalization. The plugin is configured in [`/apps/central/src/i18n.js`](/apps/central/src/i18n.js) and [`vue.config.js`](/vue.config.js).

ODK Central Frontend uses the translations stored in [`/apps/central/src/locales/`](/apps/central/src/locales/): there is a JSON file for each locale. The base name of each file must be a BCP 47 language tag, because we use the name to set the `lang` attribute of the `<html>` element. `en` is the fallback locale, and [`en.json5`](/apps/central/src/locales/en.json5) is bundled with Frontend; other files are loaded asynchronously as needed. We also make ample use of single file component `i18n` custom blocks.

We also define internationalization-related utilities in [`/apps/central/src/util/i18n.js`](/apps/central/src/util/i18n.js).
...
We use [Transifex](https://www.transifex.com/getodk/central/) to manage our translations. Our Transifex source file, [`/apps/central/transifex/strings_en.json`](/apps/central/transifex/strings_en.json), is pulled into Transifex after a commit to the master branch.
...
- You can use comments in `/apps/central/src/locales/en.json5`, which is parsed as JSON5.
- If you add a comment about a specific key to the top of `/apps/central/src/locales/en.json5`, that comment will be added for the key whether it appears in `en.json5` or a single file component `i18n` custom block.
...
- Use Vue I18n as you normally would, adding messages to `/apps/central/src/locales/en.json5` and to single file component `i18n` custom blocks.
...
Frontend implements a set of [global styles](/apps/central/src/assets/scss/app.scss). Beyond that, we use Vue single file components, so you will find a component's styles in the same file as its HTML and JavaScript.
...
The core of our tests is the tests of the components and the tests of the router, which together implement most of the business logic. We also have unit tests of composables, the Vuex store, utility functions, and so on. The directory structure of `/apps/central/test/` largely mirrors that of `/apps/central/src/`. For example, each test file in `/apps/central/test/components/` corresponds to a component in `/apps/central/src/components/`; each test file in `/apps/central/test/composables/` corresponds to a composable in `/apps/central/src/composables/`. The exception is that the files in `/apps/central/test/util/` do not test the files in `/apps/central/src/util/`, but rather are test utility functions. The files in `/apps/central/src/util/` are tested by those in `/apps/central/test/unit/`.
